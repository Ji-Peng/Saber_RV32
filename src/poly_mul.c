...
void unrolled_kara_mem_top(const uint16_t* a, const uint16_t* c, uint16_t* d)
{
    ...
    for (i = 0; i < KARA_TOP_K / 2; ++i) {
        ...
        kara_tmp_top[i] =
            d[3 * KARA_TOP_K - 1 + i] + d[3 * KARA_TOP_K + KARA_TOP_K / 2 - 1 +
                                          i];  // XXX: global variable asm
        d[2 * KARA_TOP_K + i] = 0;
    }
    for (i = 0; i < KARA_TOP_K / 2; ++i) {
    ...
    for (i = 0; i < KARA_TOP_K / 2 - 1; ++i) {
        ...
        kara_tmp_top[i] =
            c[KARA_TOP_K / 2 + i] +
            c[KARA_TOP_K + KARA_TOP_K / 2 + i];  // XXX: global variable asm
    }
    d[KARA_TOP_K + KARA_TOP_K / 2 + i] =
        d[2 * KARA_TOP_K + KARA_TOP_K / 2 - 1 + i] - d[KARA_TOP_K + i];
    d[2 * KARA_TOP_K + KARA_TOP_K / 2 - 1 + i] = 0;
    ...
    for (i = 0; i < KARA_TOP_K / 2; ++i) {
        ...
        kara_tmp_top[i] =
            c[i] + c[KARA_TOP_K / 2 + i];  // XXX: global variable asm
    }
...
void unrolled_kara_mem_bottom(const uint16_t* a, const uint16_t* c, uint16_t* d)
{
    ...
    for (i = 0; i < KARA_BOTTOM_K / 2; ++i) {
        ...
        kara_tmp[i] = d[3 * KARA_BOTTOM_K - 1 + i] +
                      d[3 * KARA_BOTTOM_K + KARA_BOTTOM_K / 2 - 1 +
                        i];  // XXX: global variable asm
    ...
    for (i = 0; i < KARA_BOTTOM_K / 2 - 1; ++i) {
        ...
        kara_tmp[i] =
            c[KARA_BOTTOM_K / 2 + i] + c[KARA_BOTTOM_K + KARA_BOTTOM_K / 2 +
                                         i];  // XXX: global variable asm
    }
    d[KARA_BOTTOM_K + KARA_BOTTOM_K / 2 + i] =
    ...
    kara_tmp[i] =
        c[KARA_BOTTOM_K / 2 + i] +
        c[KARA_BOTTOM_K + KARA_BOTTOM_K / 2 + i];  // XXX: global variable asm
    ...
    for (i = 0; i < KARA_BOTTOM_K / 2; ++i) {
        ...
        kara_tmp[i] =
            c[i] + c[KARA_BOTTOM_K / 2 + i];  // XXX: global variable asm
    }
