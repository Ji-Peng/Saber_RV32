...
uint16_t floor_special(uint16_t a)
{
    ...
    abs = sign * (0 - a) +
          (1 - sign) * a;  // absolute value of a in constant time;
    ...
    temp = -temp;  // if temp was 0, then result is 0; otherwise -ve. so the
                   // sign bit tells it.
...
void ExtractKeyBits(uint16_t *scale_data, unsigned char *scale_ar,
                    uint16_t *message_dec_unpacked)
{
    int32_t i;
    uint16_t op[SABER_N];
