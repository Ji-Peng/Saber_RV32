...
void GenSecret(uint16_t r[SABER_K][SABER_N], const unsigned char *seed)
{
    ...
    cbd(0, r[0], SHAKE128_RATE,
        shake_op_buf);  // consume all SHAKE_op bytes to construct first
                        // SHAKE128_RATE coefficients of r[0]

    keccak_squeezeblocks(shake_op_buf, 1, s, SHAKE128_RATE);
    cbd(SHAKE128_RATE, r[0], 88,
        shake_op_buf);  // consume first 88 op bytes to construct remaining
                        // coefficients of r[0]
