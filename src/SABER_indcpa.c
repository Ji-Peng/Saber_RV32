...
void InnerProd(uint16_t pkcl[SABER_K][SABER_N],uint8_t skpv[SABER_K][SABER_N],uint16_t mod,uint16_t res[SABER_N]);
void MatrixVectorMul(polyvec *a, uint8_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N], uint16_t mod, int16_t transpose);

void MatrixVectorMul_keypair(const unsigned char *seed, uint8_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N], uint16_t mod);
void MatrixVectorMul_encryption(const unsigned char *seed, uint8_t skpv[SABER_K][SABER_N], unsigned char *ciphertext, uint16_t mod);

void VectorMul(const unsigned char *seed, uint8_t skpv[SABER_K][SABER_N],uint16_t res[SABER_N]);
...
void indcpa_kem_dec(const unsigned char *sk, const unsigned char *ciphertext, unsigned char message_dec[])
{
	...
	uint8_t sksv[SABER_K][SABER_N]; //secret key of the server
	...
	if(transpose==1){
		for(i=0;i<SABER_K;i++){
			for(j=0;j<SABER_K;j++){
				...
				pol_mul((uint16_t *)&a[j].vec[i], skpv[j], res[i]);
		...
		for(i=0;i<SABER_K;i++){
			for(j=0;j<SABER_K;j++){
				...
				pol_mul((uint16_t *)&a[i].vec[j], skpv[j], res[i]);
				/*for(k=0;k<SABER_N;k++){
					res[i][k]=res[i][k]+acc[k];
...
void VectorMul(const unsigned char *byte_array, uint8_t skpv[SABER_K][SABER_N],uint16_t res[SABER_N]){
...
unsigned char MatrixVectorMul_encryption_cmp(const unsigned char *seed, uint8_t skpv[SABER_K][SABER_N], unsigned char *ciphertext, uint16_t mod)
{
