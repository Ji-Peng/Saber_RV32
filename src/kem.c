...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{
    ...
    for (i = 0; i < SABER_INDCPA_PUBLICKEYBYTES; i++)
        sk[i + SABER_INDCPA_SECRETKEYBYTES] = pk
            [i];  // sk[SABER_INDCPA_SECRETKEYBYTES:SABER_INDCPA_SECRETKEYBYTES+SABER_INDCPA_SECRETKEYBYTES-1]
                  // <-- pk
...
int crypto_kem_enc(unsigned char *c, unsigned char *k, const unsigned char *pk)
{
    ...
    sha3_256(buf + 32, pk,
             SABER_INDCPA_PUBLICKEYBYTES);  // BUF[32:63] <-- Hash(public key);
                                            // Multitarget countermeasure for
                   ...
                   const unsigned char *sk)
{
    ...
    sha3_256(kr + 32, c,
             SABER_BYTES_CCA_DEC);  // overwrite coins in kr with h(c)
