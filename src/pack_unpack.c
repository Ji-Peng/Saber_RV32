...
void pack_sk(uint8_t bytes[SABER_SKPOLYBYTES], const uint16_t data[SABER_N])
{
    int32_t j, offset_data;
    for (j = 0; j < SABER_N / 2; j++) {
        offset_data = 2 * j;
        bytes[j] = (data[offset_data + 1] << 4) | (data[offset_data] & 0xf);
    }
}
...
{
    int32_t i, j, offset_byte, offset_data;
    for (i = 0; i < SABER_L; i++) {
        for (j = 0; j < SABER_N / 2; j++) {
