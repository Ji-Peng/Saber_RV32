...
void generate_tables_256_merged(void)
{
    int32_t t;
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < 255; j++) {
            t = my_pow(root[i], tree_256_ntt_merged[j]);
            t = fqmul(t, ((int64_t)RmodM * RmodM) % M);
            // if (t > M / 2) t -= M;
            // if (t < -M / 2) t += M;
            printf("%d, ", t);
    ...
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < 255; j++) {
            t = my_pow(root[i], 512 - tree_256_intt_merged[j]);
            t = fqmul(t, ((int64_t)RmodM * RmodM) % M);
            // if (t > M / 2) t -= M;
            // if (t < -M / 2) t += M;
            printf("%d, ", t);
